...
function! QMK_KCFunction()
    let l:curr_word = expand('<cword>')
    if has_key(s:qmk_keymap, l:curr_word)
        return 'keycode for ' .. escape(s:qmk_keymap[l:curr_word] , '[]')
    endif
    return '' 
endfunction
...
def! InlineKeySybomls()

    var kc_pattern = 'KC_[_A-Z]\{3,\}'
    ...
    for row in range(1, line('$'))
        ...
        var [keycode, starts, ends] = matchstrpos(current, kc_pattern, 0, cnt)

        while starts != -1

            if has_key(s:qmk_symbol, keycode)
            ...
            cnt += 1
            [keycode, starts, ends] = matchstrpos(current, kc_pattern, 0, cnt)
        endwhile
    endfor
enddef
